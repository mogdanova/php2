...
<br>
<br>
<input type="search" placeholder="Поиск..."  name="ser1" required/><br>
<br>
					<input type="submit" value="Найти" name="sortform1">
		...
		if (isset($_GET['sortform1'])) {
			...
			while (!feof($otdelFile)) {
				...
				if (!empty($str)) {
					$temp=explode(";", $str);
					if($temp[$x]==$_GET['ser1']) $mas[$i++] = $temp;
				}
			}
			flock ($otdelFile,LOCK_SH);
			fclose($otdelFile);
...
if ($i>0){
			...
			else echo "<div class='warning'>Ничего не найдено</div>";
		}
		?>
			<h3>Книги</h3>
			<div class="sort_form">
...
<br>
<br>
<input type="search" placeholder="Поиск..."  name="ser2" required/><br>
<br>
					<input type="submit" value="Найти" name="sortform2">
		...
		if (isset($_GET['sortform2'])) {
			...
			while (!feof($bookFile)) {
				...
				if (!empty($str)) {
					$temp=explode(";", $str); 
					if(trim($temp[$x])==$_GET['ser2']) $mas[$i++] = $temp;
				}
			}
			flock ($bookFile,LOCK_SH);
			fclose($bookFile);
...
<br>
<br><input type="search"  placeholder="Поиск..." name="ser3" required/><br>
<br>
					<input type="submit" value="Найти" name="sortform3">
		...
		if (isset($_GET['sortform3'])) {
			...
			while (!feof($readersFile)) {
				...
				if (!empty($str)) {
					$temp=explode(";", $str);
					if($temp[$x]==$_GET['ser3']) $mas[$i++] = $temp;
				}
			}
			flock ($readersFile,LOCK_SH);
			fclose($readersFile);
